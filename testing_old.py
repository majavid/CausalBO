...
def timing_graph(observational_samples: pd.DataFrame, graph: SCM, exploration_set: list[list[str]], 
    ...
    D_i = pd.DataFrame(columns=['Y', 'Z'])
    D_i.append({'X': 0.0, 'Y': objective_function(0.0)}, ignore_index=True)

    # Initialize GP for interventional variable Z, SCM X->Z->Y
    Gp = SingleTaskGP(train_X=torch.tensor(D_i['X']), train_Y=torch.tensor(D_i['Y']),
    ...
    for t in range(num_steps):
        ...
        # Intervene
            ...
            global_cost += 1
            Gp.set_train_data(inputs=torch.tensor(D_i['Z']), targets=torch.tensor(D_i['Y']))
            ...
            D_i.append({'Z': new_x, 'Y': new_y}, ignore_index=True)
...
#         num_initial_obs=100,
#         num_obs_per_step=20,
#         interventional_domain=[x_bounds],
#         num_allowed_obs=1000,
#         type_trial='min',
#         objective_function=lambda x: Y(Z(X(x))))
