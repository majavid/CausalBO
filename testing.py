...
class CausalMean(Mean):
    ...
    def forward(self, x: torch.Tensor):
        ...
        mean_output = torch.tensor([self.mean_function(
                                interventional_variable=self.interventional_variable,
                                causal_model=self.causal_model,
                                interventional_value=v.tolist(), input_tensor=x_reshape) for v in x_reshape])
...
class CausalRBF2(RBFKernel):
    ...
    def __init__(self, interventional_variable, causal_model, ard_num_dims=None, batch_shape=None, active_dims=None, lengthscale_prior=None, lengthscale_constraint=None, eps=1e-06, **kwargs):
        super(CausalRBF2, self).__init__(ard_num_dims, batch_shape, active_dims, lengthscale_prior, lengthscale_constraint, eps, **kwargs)
        self.interventional_variable = interventional_variable
        self.causal_model = causal_model
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        variances_x1 = torch.tensor(np.sqrt(np.array([
            V_output_given_do(interventional_variable=self.interventional_variable,
                              interventional_value=x1_i.tolist(),
                              causal_model=self.causal_model, input_tensor= [x1, x2])
            for x1_i in x1_reshape
        ])))
        variances_x1_return = torch.reshape(variances_x1, x1_shape[:-1])
        

        variances_x2 = torch.tensor(np.sqrt(np.array([
            V_output_given_do(interventional_variable=self.interventional_variable,
                              interventional_value=x2_j.tolist(),
                              causal_model=self.causal_model, input_tensor=[x1,x2])
            for x2_j in x2_reshape
        ])))
        variances_x2_return = torch.reshape(variances_x2, x2_shape[:-1])
...
def timing_graph(observational_samples: pd.DataFrame, graph: SCM, exploration_set: list[list[str]], 
    ...
    Gp = SingleTaskGP(train_X=torch.tensor([[0.0]]), train_Y=torch.tensor([[objective_function(0.0)]]),
                                covar_module=CausalRBF(
                                    output_variable=graph.output_node,
    ...
    D_i = pd.DataFrame(columns=['X', 'Y', 'Z'])
    ...
    for t in range(num_steps):
        ...
        # Intervene
            ...
            #     Gp.set_train_data(inputs=torch.tensor([[0.0]]), targets=torch.tensor([[objective_function(0.0)]]))
            # # Standard EI acquisition function
            # Gp.set_train_data(inputs=torch.tensor([[0.0]]), targets=torch.tensor([[objective_function(0.0)]]))
            acqf = ExpectedImprovement(Gp, global_optimum)
